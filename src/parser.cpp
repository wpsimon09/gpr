...
std::optional<double> parse_double(parse_stream<string>& s)
{
    try
    {
...
std::optional<int> parse_int(parse_stream<string>& s)
{
    try
    {
        int i = stoi(s.next());
    ...
    {
        std::printf("Failed to parse string:%s , return 0 for this instruction !", s.s[s.i].c_str());
        return nullopt;
    }
}

optional<addr> parse_address(char c, parse_stream<string>& s)
{
    switch(c)
    {
        case 'X':
        ...
        case 'q':
        case 'E':
            return make_double_address(parse_double(s).value_or(0));
        case 'G':
        case 'H':
        case 'M':
        case 'N':
        ...
        case 'd':
        case 'l':
            return make_int_address(parse_int(s).value_or(0));
        default:
...
string parse_comment_with_delimiters(string sc, string ec, parse_stream<string>& s)
{
    int    depth = 0;
    string text  = "";
    do
...
chunk parse_word_address(parse_stream<string>& s)
{
    ...
    if(auto a = parse_address(c, s))
    {
        return chunk(c, a.value());
    }
    return chunk('(', ')', "Unrecognised instruction ");
}
