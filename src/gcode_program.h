...
struct comment_data
{
    ...
    comment_data(const char p_left_delim, const char p_right_delim, const std::string& p_comment_text)
...
struct word_address_data
{
    ...
    word_address_data(const char p_wd, const addr p_adr)
        : wd(p_wd)
        , adr(p_adr)
    {
    }
...
class chunk
{
  ...
  public:
    ...
    chunk(const char c)
        : chunk_tp(CHUNK_TYPE_WORD)
        , single_word(c)
    {
    }

    chunk(const char p_left_delim, const char p_right_delim, const std::string& p_comment_text)
    ...
    bool equals_word_address(const chunk& other_addr) const
    {
        ...
        return (get_word() == other_addr.get_word()) && (get_address().equals(other_addr.get_address()));
    }
    ...
    bool equals_comment(const chunk& other_comment) const
    {
        ...
        return (get_comment_text() == other_comment.get_comment_text()) && (get_left_delim() == other_comment.get_left_delim())
...
chunk make_comment(const char start_delim, const char end_delim, const std::string& comment_text);
...
class block
{
  ...
  public:
    block(const int p_line_no, const bool p_slashed_out, const std::vector<chunk> p_chunks)
        : has_line_no(true)
        , line_no(p_line_no)
...
inline block make_comment_block_from_line(const std::string& line)
{
    return block(false, {make_comment('(', ')', line)});
}
}  // namespace gpr
